...
This fork, therefore, modifies the source code directly.
Instead of generating pretty URLs, its specification has been altered to always produce URLs that explicitly include `index.html` at the end (e.g., `/about/index.html`). This approach ensures that links to pages are reliable across any environment, independent of server behavior.
